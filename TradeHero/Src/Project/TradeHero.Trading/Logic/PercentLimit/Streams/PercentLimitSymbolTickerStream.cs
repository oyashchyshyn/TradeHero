    ...
    protected override Task ManageTickerAsync(IBinance24HPrice ticker, CancellationToken cancellationToken = default)
    {
        try
        {
            Task.Run(async () =>
            {
                ...
                if (symbolInfo == null)
                {
                    Logger.LogWarning("{Symbol}. {PropertyName} is null. In {Method}", ticker.Symbol,
                        nameof(symbolInfo), nameof(ManageTickerAsync));
                    ...
                    parallelOptions, async (position, parallelCancellationToken) =>
                    {
                        ...
                        if (!positionInfo.IsNeedToCheckPosition)
                        {
                            Logger.LogDebug("{Key} with status {Status}. In {Method}",
                                key, _percentLimitStore.PositionsInfo[key].IsNeedToCheckPosition,
                                nameof(ManageTickerAsync));
                        ...
                        switch (orderToPlace)
                        {
                            ...
                            {
                                var marketClosePositionResult =
                                    await _percentLimitEndpoints.CreateMarketClosePositionOrderAsync(
                                        position,
                                        symbolInfo,
                            ...
                            case PercentLimitOrderToPlace.MarketStopToExit:
                            {
                                ...
                                {
                                    PercentLimitOrderToPlace.MarketStopToSafe => _percentLimitStore
                                        .TradeLogicLogicOptions
                                        .MarketStopSafePriceFromLastPricePercent ?? 0.0m,
                                    PercentLimitOrderToPlace.MarketStopToExit => _percentLimitStore
                                        .TradeLogicLogicOptions
