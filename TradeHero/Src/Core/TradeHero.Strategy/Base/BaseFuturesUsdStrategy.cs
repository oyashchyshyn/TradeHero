...
internal abstract class BaseFuturesUsdStrategy : IStrategy
{
    ...
    public IStrategyStore Store { get; protected init; } = null!;

    protected BaseFuturesUsdStrategy(
    ...
    public async Task<ActionResult> InitAsync(StrategyDto strategyDto)
    {
        try
        {
            ...
            if (instanceResult != null)
            {
                ...
                actionResult = ((BaseStrategyStore)Store).AddInstanceOptions(strategyDto, instanceResult.Type);
                if (actionResult != ActionResult.Success)
                {
                    return actionResult;
                }
    ...
    public async Task<ActionResult> FinishAsync(bool isNeedToUseCancellationToken)
    {
        try
        {
            ...
            if (((BaseStrategyStore)Store).UsdFuturesTickerStreams.Any())
            {
                foreach (var usdFuturesTickerStream in ((BaseStrategyStore)Store).UsdFuturesTickerStreams)
                {
            ...
            await ((BaseStrategyStore)Store).ClearDataAsync();
    ...
    {
        var instance = ((BaseStrategyStore)Store).InstanceOptions;
    ...
    private void SetBackgroundJobs(IStrategyStore store, CancellationToken cancellationToken)
    {
