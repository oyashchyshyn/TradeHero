...
internal class PmsPositionWorker : BasePositionWorker
{
    private readonly ILogger<PmsPositionWorker> _logger;
    ...
    private readonly PmsSymbolTickerStreamFactory _pmsSymbolTickerStreamFactory;

    public PmsPositionWorker(
        ILogger<PmsPositionWorker> logger,
        IThRestBinanceClient restBinanceClient, 
        IThSocketBinanceClient socketBinanceClient,
        IDateTimeService dateTimeService, 
        PmsSymbolTickerStreamFactory pmsSymbolTickerStreamFactory
        )
    {
        _logger = logger;
        ...
        _pmsSymbolTickerStreamFactory = pmsSymbolTickerStreamFactory;
    }

    public override async Task<ActionResult> CreatePositionAsync(IStrategyStore strategyStore, string symbol, PositionSide side, decimal entryPrice, 
        DateTime lastUpdateTime, decimal quantity, bool isPositionExist, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var pmsStore = (PmsStore)strategyStore;
            ...
            {
                var stream = _pmsSymbolTickerStreamFactory.GetPmsSymbolTickerStream();
    ...
    public override ActionResult UpdatePositionDetails(IStrategyStore strategyStore, Position openedPosition, 
    ...
    public void UpdatePositionQuantity(IStrategyStore strategyStore, Position openedPosition, 
        BinanceFuturesStreamOrderUpdate orderUpdate, bool isWithdraw)
    {
        try
        {
            ...
            AddOrUpdateOrderPrice((PmsStore)strategyStore, openedPosition.Name, orderUpdate.UpdateData.AveragePrice);
        }
        catch (Exception exception)
        {
            _logger.LogCritical(exception, "{Position}. In {Method}", 
    ...
    public override async Task<ActionResult> DeletePositionAsync(IStrategyStore strategyStore, Position positionToDelete, 
    ...
    private void AddOrUpdateOrderPrice(PmsStore pmsStore, string symbol, decimal lastOrderPrice)
    {
        try
        {
            if (pmsStore.SymbolLastOrderPrice.ContainsKey(symbol))
            {
                pmsStore.SymbolLastOrderPrice[symbol] = lastOrderPrice;
            }
            else
            {
                pmsStore.SymbolLastOrderPrice.Add(symbol, lastOrderPrice);   
            }
        }
        catch (Exception exception)
        {
