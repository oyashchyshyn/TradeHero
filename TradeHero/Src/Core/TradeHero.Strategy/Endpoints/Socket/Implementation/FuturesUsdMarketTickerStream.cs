...
internal class FuturesUsdMarketTickerStream : IFuturesUsdMarketTickerStream
{
    ...
    public async Task<ActionResult> StartStreamMarketTickerAsync(IStrategyStore store, int maxRetries = 5, CancellationToken cancellationToken = default)
    {
        try
        {
            for (var i = 0; i < maxRetries; i++)
            {
                ...
                void OnMessage(DataEvent<IEnumerable<IBinance24HPrice>> onMessage)
                {
                    ...
                    {
                        if (((BaseStrategyStore)store).MarketLastPrices.ContainsKey(binance24HPrice.Symbol))
                        {
                            ((BaseStrategyStore)store).MarketLastPrices[binance24HPrice.Symbol] = binance24HPrice.LastPrice;
                        }
                        else
                        {
                            ((BaseStrategyStore)store).MarketLastPrices.Add(binance24HPrice.Symbol, binance24HPrice.LastPrice);
                        }
                    }
                }
